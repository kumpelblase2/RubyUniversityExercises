...
class Day
  ...
  def to_representation(in_representation)
    if in_representation.day_num? then self.to_day_num()
    elsif in_representation.day_sym? then self.to_day_sym()
...
class DaySym < Day
  ...
  def to_day_num()
    DayNum[to_external(DAYSYM.index(self.sym))]
  end
  def shift(in_shift)
...
class DayNum < Day
  ...
  def to_day_sym()
    DaySym[DAYSYM[to_internal(self.num)]]
  end
  def shift(in_shift)
    check_pre((in_shift.int?))
    DayNum[to_external(to_internal(self.num + in_shift) % WEEK_END)]
  end
